...
//		System.out.println("주차요금 : " + parkingCost +"원");
		...
		// 해설
		 ...
		 pHour =  time.nextInt(); // 주차 시간 입력
		 pMin = time.nextInt(); // 주차 분 입력
		 totalTime = pHour*60 + pMin; // 총 주차 시간(분) - 기본시간을 제외한 나머지
		 ...
		 totalTime = 0;
			 ...
			 cost = 1700;
			 totalTime -= 120; // 2시간 기본 시간 뺴기
		 } else { // 2시간 미만 주차시
			 totalTime = totalTime<30 ? 0 : totalTime - 30; // 30분 기본 시간 뺴기
		 }
